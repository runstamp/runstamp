...
export async function openPptx(buffer, limits) {
    ...
    if (entries.length > maxEntries) {
        throw new Error(`openPptx: archive has ${entries.length} entries, exceeding the limit of ${maxEntries}`);
    }
    let totalBytes = 0;
    for (const entry of entries) {
        ...
        if (totalBytes > maxTotalBytes) {
            throw new Error(`openPptx: total uncompressed size exceeded the limit of ${maxTotalBytes} bytes while inflating "${entry.name}"`);
        }
