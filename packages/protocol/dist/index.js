...
export { COMPOSITION_PRIMITIVE_NAMES, CompositionBlockSchema, buildCompositionBlocks, } from "./composition.js";
export { MIN_REGION_STATIC, MIN_REGION_VARIABLE, minRegionFor, remediationFor, } from "./minRegion.js";
...
export const BrandRefSchema = z
    ...
    .refine((value) => Boolean(value.brandPackId || value.brandPackVersionId), "brandPackId or brandPackVersionId is required when brand is supplied");
...
}).refine((data) => data.rows.every((r) => r.values.length === data.columns.length - 1), { message: "Each row must have exactly columns.length - 1 values (first column is the label header)" });
