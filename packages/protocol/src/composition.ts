...
const TextBlockInputSchema = z.strictObject({
  ...
  lineHeight: z
    ...
    .optional()
    .describe(
      "Line height as a multiple of font size (e.g. 1.4 for 1.4× spacing). Values ≥ 4 are deprecated legacy points.",
    ),
...
// ---------------------------------------------------------------------------
// Primitive registry — name → { schema, fn }.
// Adding a new primitive: write its input schema above, register it here.
// ---------------------------------------------------------------------------
...
} as const satisfies Record<string, PrimitiveEntry>;

export type CompositionPrimitiveName = keyof typeof PRIMITIVE_REGISTRY;

export const COMPOSITION_PRIMITIVE_NAMES = Object.keys(
  PRIMITIVE_REGISTRY,
) as CompositionPrimitiveName[];
...
const RegionGridSchema = z.strictObject({
  ...
  { message: "Region escapes the 12×12 grid (col+colSpan or row+rowSpan > 12)" },
);
...
const blockVariant = <K extends CompositionPrimitiveName>(name: K) =>
  z.strictObject({
...
const ContainerBlockSchema: z.ZodType<ContainerBlock> = z.lazy(() =>
  z.strictObject({
...
export const CompositionBlockSchema: z.ZodType<CompositionBlock> = z.lazy(() =>
  z.union([RegisteredBlockSchema, ContainerBlockSchema]),
) as z.ZodType<CompositionBlock>;

// ---------------------------------------------------------------------------
// Grid resolution + dispatch.
// ---------------------------------------------------------------------------
...
export function buildCompositionBlocks(
  blocks: CompositionBlock[],
  ...
  const applyBlockZIndex = (node: PrimitiveNode, zIndex: number | undefined): PrimitiveNode => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (block.primitive === "container") {
      ...
      const input = block.input as ContainerInput;
      const pad = input.padding ?? 0;
      const subCanvas: CompositionCanvas = {
        left: rect.left + pad,
        top: rect.top + pad,
    ...
    const entry = PRIMITIVE_REGISTRY[block.primitive as CompositionPrimitiveName];
    const key = `${block.primitive}_${i}`;
    const result = entry.fn(block.input as never, tokens, rect);
