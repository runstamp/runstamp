  ...
  for (const family of families) {
    ...
    if (strategy === "system" || strategy === "user-embedded") {
      ...
      try {
        ...
        if (strict) {
          throw new PaperError(
            `Font "${family}" not loaded and portable fallback failed: ${e.message}. Call loadFont("${family}", buffer) before rendering.`,
            { code: "FONT_NOT_FOUND", phase: "font" }
          );
        }
        getLogger().warn(`[autoFont] Font "${family}" unavailable and portable fallback failed: ${e.message}. Measurement will use char-count estimate.`);
      }
    }
  }
...
//# sourceMappingURL=chunk-MVPJ57UB.js.map
