// src/ooxml/theme.ts
// Dynamic Office Theme generation with optional ThemeConfig overrides

import type { ThemeConfig } from "../types/ast.js";
import { escapeXml } from "./drawing/textUtils.js";

// Default color values (used when no override is provided)
...
/**
 * Strip leading '#' from hex color string if present.
 */
...
/**
 * Emit a color slot element. dk1 and lt1 have special sysClr defaults;
 * all others default to srgbClr.
 */
function emitColorSlot(
  slot: string,
  ...
  // All other slots default to srgbClr with their default values
  const defaultVal = DEFAULT_COLORS[slot];
  return `<a:${slot}><a:srgbClr val="${defaultVal}"/></a:${slot}>`;
}

export function generateTheme(themeConfig?: ThemeConfig): string {
  ...
  const colorSchemeXml = colorSlots
    .map((slot) => emitColorSlot(slot, cs?.[slot as keyof typeof cs]))
    .join("\n      ");

  // Font scheme — major font
  const majorLatinXml = fs?.majorLatin
    ? `<a:latin typeface="${escapeXml(fs.majorLatin)}"/>`
    : `<a:latin typeface="Carlito"/>`;

  const majorEaXml = fs?.majorEa
    ? `<a:ea typeface="${escapeXml(fs.majorEa)}"/>`
    : `<a:ea typeface=""/>`;

  // Font scheme — minor font
  const minorLatinXml = fs?.minorLatin
    ? `<a:latin typeface="${escapeXml(fs.minorLatin)}"/>`
    : `<a:latin typeface="Carlito"/>`;

  const minorEaXml = fs?.minorEa
    ? `<a:ea typeface="${escapeXml(fs.minorEa)}"/>`
    : `<a:ea typeface=""/>`;
...
// PowerPoint for Mac is noticeably stricter about notes parts than the main slide
// package. Using a dedicated Office notes theme mirrors what PowerPoint itself
// writes when notes are authored and avoids repair prompts on Mac.
export function generateNotesTheme(): string {
  // Batch E owns the remaining script-specific slots; only admitted Latin defaults change here.
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
