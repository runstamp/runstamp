...
export const STRICT_LAYOUT_VALIDATION = "error" as const;
 ...
 *   });
 */
export function primitiveCompilerEngineOptions(
  onDiagnostic?: (diagnostic: LayoutDiagnostic) => void,
): {
  layoutValidation: typeof STRICT_LAYOUT_VALIDATION;
  onLayoutWarning: (warning: { slide: number; code: string; path: string; message: string }) => void;
} {
  return {
    layoutValidation: STRICT_LAYOUT_VALIDATION,
