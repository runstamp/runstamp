...
import type { PrimitiveNode, Rect, TextNode, ViewNode } from "../layout/types.js";
...
type SegmentName = ResolvedTokens["chrome"]["footer"]["layout"][number];
...
export const footerChrome: Primitive<FooterChromeInput> = (input, tokens, region) => {
...
type Segment =
  | { kind: "spacer"; measuredWidth: 0; build: null }
  | {
      kind: "text" | "pageNumber" | "watermarkImage";
      measuredWidth: number;
      build: (rect: Rect) => PrimitiveNode;
    };
...
function scaledCaption(
  tokens: ResolvedTokens,
  scale: number,
): ResolvedTokens["type"]["caption"] {
...
function makePageNumberSegment(
  index: number,
  tokens: ResolvedTokens,
  scale: number,
): Segment {
...
function makeCircledAccentNumber(
  rect: Rect,
  label: string,
  diameter: number,
  tokens: ResolvedTokens,
...
function makeBoxedAccentNumber(
  rect: Rect,
  label: string,
  side: number,
  tokens: ResolvedTokens,
...
function resolveColorRole(
  role: "foreground" | "muted" | "faint" | "accent" | "surface",
  tokens: ResolvedTokens,
): string {
