...
import type { PrimitiveNode, Rect, TextRun } from "../layout/index.js";
import { textBlock } from "./textBlock.js";
import { bulletList, type BulletItem } from "./bulletList.js";
import {
  estimateLineCount,
  estimateLineHeight,
...
export interface InfoCardInput {
  ...
  fill?: FillRole | string;
  /** Card border. */
  border?: { color?: BorderRole | string; width?: number };
...
export const infoCard: Primitive<InfoCardInput> = (input, tokens, region) => {
  ...
  const remainingHeight = () =>
    Math.max(0, innerRect.top + innerRect.height - cursor);
  ...
  if (input.footer) {
    ...
    const footerText =
      typeof input.footer.text === "string"
        ? input.footer.text
  ...
  const bodyAvailable = Math.max(
    0,
    remainingHeight() - (footerHeight > 0 ? footerHeight + gap : 0),
  );
  const bodyTopRect: Rect = {
    left: innerRect.left,
    top: cursor,
    width: innerRect.width,
  ...
  const overflow: PrimitiveResult["overflow"] =
    bodyResult.overflow.kind === "paginated"
      ? bodyResult.overflow
      : { kind: "fit" };
