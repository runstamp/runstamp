...
export const titleBlock: Primitive<TitleBlockInput> = (input, tokens, region) => {
  ...
  // Eyebrow.
  if (input.eyebrow) {
    const { node, consumedHeight } = emitLine(
      input.eyebrow,
      "eyebrow",
      tokens,
  ...
  let scale = 1.0;
  let titleNodeResult: EmittedTitle | null = null;
  ...
  let overflow: PrimitiveResult["overflow"] = { kind: "fit" };
  if (!titleNodeResult) {
...
function layoutTitle(
  ...
  const lineHeightPt =
    role.lineHeight !== undefined ? role.lineHeight * scale : sizePt * 1.2;
...
function emitLine(
  ...
  color: string,
  place: { left: number; top: number; width: number },
): { node: TextNode; consumedHeight: number } {
