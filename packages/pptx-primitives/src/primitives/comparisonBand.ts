...
export const comparisonBand: Primitive<ComparisonBandInput> = (input, tokens, region) => {
  ...
  const columnLeft = (idx: number): number =>
    idx === 0 ? region.left : region.left + labelColWidth + (idx - 1) * valueColWidth;
  const columnWidth = (idx: number): number => (idx === 0 ? labelColWidth : valueColWidth);
  ...
  const headerRule = emitHorizontalRule(
...
function computeRowHeight(
  row: ComparisonBandInput["rows"][number],
