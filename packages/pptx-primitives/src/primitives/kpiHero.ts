...
export const kpiHero: Primitive<KpiHeroInput> = (input, tokens, region) => {
  ...
  while (valueWidth > region.width && valueScale > VALUE_MIN_COMPRESSION - 1e-9) {
    ...
    valueWidth = estimateValueWidth(
      input.value,
      display.family,
      display.size * valueScale,
      display.letterSpacing * valueScale,
      tokens,
    );
  }
  const valueSizePt = display.size * valueScale;
  const valueLineHeightPt =
    display.lineHeight !== undefined ? display.lineHeight * valueScale : valueSizePt * 1.05;
  ...
  if (input.delta) blockHeight += tokens.spacing.sm + captionHeight;
  if (input.support) blockHeight += tokens.spacing.sm + supportHeight;
  ...
  const overflowKind: PrimitiveResult["overflow"] =
    cursor > regionBottom
      ? {
          kind: "clipped",
          droppedCount: 0,
