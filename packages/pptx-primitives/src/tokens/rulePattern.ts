...
const STYLE_VALUES = new Set(["solid", "dashed", "dotted"] as const);
type RuleStyle = "solid" | "dashed" | "dotted";
...
export function parseRulePattern(
  ...
  const totalHeight =
    lines.reduce((acc, l) => acc + l.width, 0) + gap * Math.max(0, lines.length - 1);
...
function parseLine(
  ...
  return { width: widthNum, style: style as RuleStyle, color };
}
...
function resolveColor(
  ...
  if (raw.startsWith("token:")) {
    ...
    const value = (palette as Record<string, string | null>)[role];
