...
import type { ResolvedTokens, TokenBundle } from "./schema.js";
...
export interface TokenLintIssue {
  code:
    ...
    | "TOKEN_FONT_UNPINNED"
    | "TOKEN_CONTRAST_LOW"
    | "TOKEN_SPACING_SCALE_INCONSISTENT"
    | "TOKEN_TYPE_SCALE_INCONSISTENT"
    | "TOKEN_LINE_HEIGHT_TIGHT";
...
const CONTRAST_CHECKS: Array<{
  path: string;
  foreground: keyof ResolvedTokens["palette"] | "canvas.surface";
  background: keyof ResolvedTokens["palette"] | "canvas.surface";
  minimum: number;
}> = [
...
export function lintTokenBundle(input: TokenBundle | unknown, options: TokenLintOptions = {}): TokenLintReport {
  ...
  try {
    ...
    if (err instanceof TokenResolveError) {
      issues.push(
        ...err.issues.map((issue): TokenLintIssue => ({
...
function buildReport(issues: TokenLintIssue[], options: TokenLintOptions): TokenLintReport {
...
function lintContrast(tokens: ResolvedTokens): TokenLintIssue[] {
  const issues: TokenLintIssue[] = [];
...
function lintSpacing(tokens: ResolvedTokens): TokenLintIssue[] {
  const steps: Array<[keyof ResolvedTokens["spacing"], number]> = [
...
function lintTypeScale(tokens: ResolvedTokens): TokenLintIssue[] {
...
function lintLineHeight(tokens: ResolvedTokens): TokenLintIssue[] {
  const issues: TokenLintIssue[] = [];
...
function readTokenColor(tokens: ResolvedTokens, path: keyof ResolvedTokens["palette"] | "canvas.surface"): string {
  if (path === "canvas.surface") return tokens.canvas.surface;
