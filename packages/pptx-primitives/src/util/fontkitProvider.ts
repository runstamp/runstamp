...
import type { GlyphMetrics, MetricsProvider } from "./metricsProvider.js";
...
interface FaceCacheEntry {
  font: fontkit.Font;
  unitsPerEm: number;
  /** lineHeight in EM units: ascent − descent + lineGap. */
  lineHeightUnits: number;
  /** Cached avg width ratio (advance per pt at 96/72 px-per-pt). */
  avgWidthRatio: number;
  /** Ascender in points / unitsPerEm (multiply by sizePt for px-per-pt). */
  ascenderRatio: number;
  /** Descender as a positive ratio. */
  descenderRatio: number;
}

function createFace(buffer: Buffer | Uint8Array): FaceCacheEntry {
  // fontkit accepts a Buffer; coerce Uint8Array if the caller passed one.
  const buf = Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer);
  const raw = fontkit.create(buf) as fontkit.Font | fontkit.FontCollection;
  const font = "fonts" in raw ? (raw.fonts[0] as fontkit.Font) : raw;
  ...
  let avgWidthUnits = 0;
  const os2 = (font as unknown as Record<string, unknown>)["OS/2"] as
 ...
 */
export function buildFontkitMetricsProvider(
  fonts: FontkitFontEntry[],
): MetricsProvider {
  // Family → preferred face. We stash by-style faces too in case future
  // primitives want them; today the lookup is family-only.
  const families = new Map<string, FaceCacheEntry>();

  for (const entry of fonts) {
    let face: FaceCacheEntry;
    try {
  ...
  return (family: string): GlyphMetrics | null => {
