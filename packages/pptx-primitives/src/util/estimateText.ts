 ...
 */
const WIDTH_RATIO_BY_FAMILY: Record<string, number> = {
  "Helvetica Neue": 0.50,
  "Helvetica": 0.50,
  "Arial": 0.52,
  "Inter": 0.49,
...
function resolveProvider(source: MetricsSource): MetricsProvider | null {
  if (!source) return null;
  if (typeof source === "function") return source;
  return getMetricsProvider(source);
}

function getMetrics(source: MetricsSource, family: string): GlyphMetrics | null {
...
export function estimateLineHeight(
  ...
  if (lineHeightPt !== undefined) return lineHeightPt * PX_PER_PT;
  if (source && family) {
...
export function estimateLineCount(
  input: TextMeasureInput & { width: number },
  source?: MetricsSource,
): number {
  ...
  let measureWord: (word: string) => number;
  let spaceWidth: number;
  if (metrics?.measureWidthPx) {
    measureWord = (word) =>
      metrics.measureWidthPx!(input.uppercase ? word.toUpperCase() : word, input.sizePt) +
    ...
    measureWord = (word) =>
      word.length * input.sizePt * PX_PER_PT * adjusted +
      Math.max(0, word.length - 1) * tracking;
...
function splitOnHyphens(chunk: string): string[] {
 ...
 */
export function measureByRole(
  content: string,
  role: keyof ResolvedTokens["type"],
  tokens: ResolvedTokens,
  opts: { width?: number } = {},
): { width: number; lines: number; lineHeight: number } {
...
export function estimateRunsWidth(
  runs: TextRun[],
  base: { family: string; sizePt: number; letterSpacing?: number; uppercase?: boolean },
  source?: MetricsSource,
): number {
  let total = 0;
...
export function applyTypeTransform(
  content: string,
  transform: ResolvedTokens["type"]["display"]["transform"],
): string {
  switch (transform) {
    case "upper":
