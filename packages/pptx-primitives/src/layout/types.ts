...
export type BulletAutoNumScheme =
...
// ---------------------------------------------------------------------------
// Table primitives (Phase 8) — real OOXML <a:tbl> emission.
//
// Maps to PaperTable / TableData / TableCell in the engine. Cell content
// can be a flat string (sugar), runs (rich text), or paragraphs (bullets).
// Column widths are pixel-explicit; row heights default to "fit content"
// when not set, with `minHeight` as a floor.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Connector primitives (Phase 8) — real OOXML <p:cxnSp> emission.
//
// Used for leader lines and pointer arrows. Endpoints are pixel coords;
// connectors are not anchored to shape IDs from the primitive layer —
// the layer above (composition) is responsible for picking endpoints
// that align with the visual targets.
// ---------------------------------------------------------------------------
