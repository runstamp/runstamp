 ...
 */
export const BUNDLED_FONT_POOL = Object.freeze([
    "Arial",
    "Calibri",
    "Helvetica",
    "Helvetica Neue",
 ...
 * Never throws.
 */
export function auditFontAvailability(tokens) {
    const embeddedFamilies = new Set(tokens.embeddedFonts.map((f) => normalize(f.family)));
    ...
    for (const [roleName, role] of Object.entries(tokens.type)) {
        ...
        warnings.push({
            ...
            message: `Font "${role.family}" is neither on the engine's system-font map nor ` +
...
export function isBundledFont(family) {
...
export function findEmbeddedRegular(family, fonts) {
    const f = normalize(family);
    for (const font of fonts) {
        if (normalize(font.family) !== f)
            continue;
