...
export const metricStack = (input, tokens, region) => {
    const nodes = [];
    ...
    for (let i = startIndex; i < input.rows.length; i++) {
        ...
        const ruleHeightAfter = i < input.rows.length - 1 && tokens.rules.divider !== "none"
...
function layoutRow(row, tokens, place) {
    ...
    const valueLineHeightPt = valueRole.lineHeight !== undefined ? valueRole.lineHeight * scale : valueSizePt * 1.15;
...
//# sourceMappingURL=metricStack.js.map