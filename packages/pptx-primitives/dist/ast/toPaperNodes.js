 ...
 */
export function toPaperNodes(nodes) {
    return nodes.map(toPaperNode);
}
export function toPaperNode(node) {
    switch (node.kind) {
        case "view":
...
function translateRun(run) {
    const runStyle = {};
    if (run.bold)
        runStyle.fontWeight = "bold";
    if (run.italic)
        runStyle.fontStyle = "italic";
    if (run.underline)
        runStyle.textDecorationLine = "underline";
    if (run.color !== undefined)
        runStyle.color = run.color;
    if (run.fontSize !== undefined)
        runStyle.fontSize = run.fontSize * PT_TO_PX;
    if (run.fontFamily !== undefined)
        runStyle.fontFamily = run.fontFamily;
...
function translateParagraph(para) {
    ...
    if (para.align !== undefined)
        out.align = para.align;
    if (para.level !== undefined)
        out.level = para.level;
    if (para.indent !== undefined)
        out.indent = para.indent;
    if (para.marginLeft !== undefined)
        out.marginLeft = para.marginLeft;
    if (para.hangingIndent !== undefined)
        out.hangingIndent = para.hangingIndent;
    if (para.spaceBefore !== undefined)
        out.spaceBefore = para.spaceBefore;
    if (para.spaceAfter !== undefined)
        out.spaceAfter = para.spaceAfter;
    if (para.bullet !== undefined)
        out.bullet = para.bullet;
    return out;
}
...
function translateChart(node) {
    return {
        ...
        decorative: node.decorative ?? false,
        style: withAbsoluteRect(node.zIndex !== undefined ? { zIndex: node.zIndex } : {}, node.rect),
    };
}
...
function translateTableCell(cell) {
    ...
    if (cell.style) {
        const style = {};
        if (cell.style.fill !== undefined)
            style.fill = cell.style.fill;
        if (cell.style.borders !== undefined)
            style.borders = cell.style.borders;
        if (cell.style.fontWeight !== undefined)
            style.fontWeight = cell.style.fontWeight;
        if (cell.style.fontStyle !== undefined)
            style.fontStyle = cell.style.fontStyle;
        if (cell.style.fontSize !== undefined)
            style.fontSize = cell.style.fontSize * PT_TO_PX;
...
function translateTable(node) {
    ...
    if (node.borders) {
        const ts = {};
        if (node.borders.outer)
            ts.outerBorder = node.borders.outer;
        if (node.borders.innerH)
            ts.innerBorderH = node.borders.innerH;
        if (node.borders.innerV)
            ts.innerBorderV = node.borders.innerV;
        tableData.style = ts;
    }
    return {
        type: "Table",
...
function translateConnector(node) {
    ...
    if (node.lineWidth !== undefined)
        out.lineWidth = node.lineWidth;
    if (node.lineColor !== undefined)
        out.lineColor = node.lineColor;
    if (node.lineDashStyle !== undefined)
        out.lineDashStyle = node.lineDashStyle;
    if (node.arrowStart)
        out.arrowStart = true;
    if (node.arrowEnd)
        out.arrowEnd = true;
...
//# sourceMappingURL=toPaperNodes.js.map