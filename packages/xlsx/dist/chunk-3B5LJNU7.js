// src/fixtures/phase1.ts
function createRow(values) {
  return {
    cells: values.map((value) => ({ value }))
  };
}
function createLargeWorkbook(rowCount, colCount, sheetName = "Data") {
...
function createMediumWorkbook() {
  const productNames = ["Platform", "API", "Analytics", "Support", "Enterprise"];
  const regions = ["NA", "EMEA", "APAC"];
  const sheets = ["Revenue", "Pipeline", "Retention"].map((sheetName, sheetIndex) => ({
    name: sheetName,
    rows: [
      ...
      sheets: [{
        ...
        rows: [
          ...
          ...Array.from({ length: 19 }, (_unused, index) => createRow([
            `\u65E5\u672C\u8A9E\u30B7\u30FC\u30C8 ${index + 1}`,
            `\u0645\u0631\u062D\u0628\u0627 ${index + 1}`,
            `\u{1F44B}\u{1F3FB} cafe\u0301 family-${index + 1} \u200D join`
          ]))
        ]
      }]
    },
    ...
    document: {
      sheets: [{
        ...
        rows: Array.from({ length: 10 }, (_unused, index) => createRow([
          ...
          `bad\btext ${index} &amp; "quotes" 'apos' <![CDATA[test]]>`
        ]))
      }]
    },
    rows: 10,
...
line ${index + 1}`, "	"]))
  ...
  "multi-sheet": {
    ...
    document: {
      sheets: [
        { name: "Revenue", rows: Array.from({ length: 10 }, (_unused, index) => createRow([`Q${index + 1}`, 1e3 + index, index % 2 === 0])) },
        { name: "Pipeline", rows: Array.from({ length: 20 }, (_unused, index) => createRow([`Deal ${index + 1}`, 5e4 + index * 100, "Open", new Date(Date.UTC(2026, 0, index % 28 + 1)), null])) },
  ...
  "large-10k": {
    ...
    document: createLargeWorkbook(1e4, 20, "Large10K"),
    rows: 1e4,
    cols: 20,
    sheets: 1
  },
  "large-50k": {
    ...
    document: createLargeWorkbook(5e4, 20, "Large50K"),
    rows: 5e4,
    cols: 20,
    sheets: 1
  },
  "large-100k": {
    ...
    document: createLargeWorkbook(1e5, 10, "Large100K"),
    rows: 1e5,
    cols: 10,
    sheets: 1
  },
...
function getPhase1Fixture(name) {
...
//# sourceMappingURL=chunk-3B5LJNU7.js.map
