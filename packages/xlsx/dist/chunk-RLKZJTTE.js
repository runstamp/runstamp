...
function renderStageTotalMs(stageMetrics) {
  return stageMetrics.worksheetSerializationTimeMs + stageMetrics.stylesSerializationTimeMs + stageMetrics.sharedStringsSerializationTimeMs + stageMetrics.packageSerializationTimeMs + stageMetrics.archiveFinalizationTimeMs;
}
...
function classifyTimingVarianceStatus(value, max) {
  if (value <= max) {
    return "pass";
  }
  return value <= max * 1.1 ? "warn" : "fail";
}
function classifyRenderLatencyStatus(measuredP95, targetMs, diagnosticStageMetrics) {
...
var defaultTempExcelJsRequireBase = "/tmp/runstamp-xlsx-bench-deps/package.json";
...
async function runRenderBenchmarks(iterations) {
  ...
  const styleOverheadStatus = styleOverheadRatio <= 1.5 ? "pass" : styled10kStatus === "warn" ? "warn" : "fail";
  return [
    benchmarkResult(
      "F5",
      "F",
  ...
  if (excelJsLoad.status === "loaded") {
    try {
      ...
      excelJsResult = benchmarkResult(
        ...
        `Runstamp ${formatKb(styled10kBuffer.length)}; ExcelJS ${formatKb(excelJsBuffer.length)}; ratio ${ratio.toFixed(2)}\xD7`,
...
//# sourceMappingURL=chunk-RLKZJTTE.js.map
